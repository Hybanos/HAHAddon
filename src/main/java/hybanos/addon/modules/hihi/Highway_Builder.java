...
public class Highway_Builder extends Module {
    public enum Floor {
        Replace,
        PlaceMissing
    }
    ...
    private final Setting<List<Block>> replaceable = sgGeneral.add(new BlockListSetting.Builder()
        .name("blocks to replace")
        .description("The blocks to replace.")
        .defaultValue(Blocks.SOUL_SAND, Blocks.RED_MUSHROOM, Blocks.BROWN_MUSHROOM, Blocks.WEEPING_VINES, Blocks.TWISTING_VINES)
        .build()
    );
    ...
    // private final Setting<Boolean> railingsStartAtFloor = sgGeneral.add(new BoolSetting.Builder()
    //     .name("railings at floor")
    //     .description("places support blocks for diagonal railings at floor level.")
    //     .visible(railings::get)
    //     .defaultValue(true)
    //     .build()
    // );
    ...
    private final Setting<Boolean> bypass = sgGeneral.add(new BoolSetting.Builder()
        .name("bypass")
        .description("forces item slot switching.")
        .defaultValue(true)
        .build()
    );

    private final Setting<Integer> reset = sgGeneral.add(new IntSetting.Builder()
        .name("reset timer")
        .description("The number of ticks to wait before reset when stalling.")
        .defaultValue(100)
        .sliderRange(20, 200)
        .build()
    );
    ...
    public Highway_Builder() {
        super(HAHAddon.SKID, "Highway Builder", "The Basto special.");
    }
    ...
    private enum State {
        Center {
            @Override
            protected void tick(Highway_Builder b) {
        ...
        Forward {
            ...
            protected void tick(Highway_Builder b) {
                ...
                else b.input.forward(true); // Move
            }

            private boolean needsToMine(Highway_Builder b, MBPIterator it, boolean ignoreBlocksToPlace) {
                boolean found = false;

                if (!ignoreBlocksToPlace && !b.replaceable.get().isEmpty()) {
                    for (MBlockPos pos : it) {
                        if (b.replaceable.get().contains(pos.getState().getBlock())) found = true;
                    }
                    b.info("" + found);
                    return found;
                }
            ...
            private boolean needsToPlace(Highway_Builder b, MBPIterator it, boolean liquids) {
                for (MBlockPos pos : it) {
        ...
        ThrowOutTrash {
            ...
            @Override
            protected void start(Highway_Builder b) {
                int biggestCount = 0;
        ...
        MineEnderChests {
            ...
            protected void tick(Highway_Builder b) {
                ...
                if (blockState.getBlock() == Blocks.ENDER_CHEST) {
                    ...
                    BlockUtils.breakBlock(pos.getBlockPos(), true);
                }
                else {
                    // Place ender chest
                    ...
                    BlockUtils.place(pos.getBlockPos(), Hand.MAIN_HAND, slot, true, 0, true, true, false);
                }
            }
        };
        ...
        protected abstract void tick(Highway_Builder b);

        protected void checkState(Highway_Builder b) {
            b.timeSinceLastSwitch++;

            if (b.timeSinceLastSwitch > b.reset.get() && !b.next && b.reset.get() != 0) {
                InvUtils.swap(1, false);
                b.next = true;
                return;
            } else if (b.next) {
                InvUtils.swap(2, false);
                b.next = false;
                b.timeSinceLastSwitch = 0;
                return;
            }
        }

        protected void mine(Highway_Builder b, MBPIterator it, boolean ignoreBlocksToPlace, State nextState, State lastState) {
            boolean breaking = false;

            if (b.bypass.get() && b.needsToWait) {
                b.needsToWait = false;
                for (MBlockPos pos : it) {
                    BlockState state = pos.getState();
            ...
            for (MBlockPos pos : it) {
                BlockState state = pos.getState();
                if (state.isAir() || (!ignoreBlocksToPlace && b.blocksToPlace.get().contains(state.getBlock()))) continue;
                if (!b.replaceable.get().isEmpty() && !b.replaceable.get().contains(state.getBlock()) && !ignoreBlocksToPlace) continue;
        ...
        protected void mine(Highway_Builder b, MBPIterator it, boolean ignoreBlocksToPlace) {
            mine(b, it, ignoreBlocksToPlace, Forward, b.state);
        }

        protected void place(Highway_Builder b, MBPIterator it, int slot, State nextState) {
        ...
        private int findSlot(Highway_Builder b, Predicate<ItemStack> predicate, boolean hotbar) {
        ...
        private int findHotbarSlot(Highway_Builder b, boolean replaceTools) {
            int thrashSlot = -1;
        ...
        private boolean hasItem(Highway_Builder b, Item item) {
            for (int i = 0; i < b.mc.player.getInventory().getMainStacks().size(); i++) {
        ...
        protected int findAndMoveToHotbar(Highway_Builder b, Predicate<ItemStack> predicate, boolean required) {
            // Check hotbar
        ...
        protected int findAndMoveBestToolToHotbar(Highway_Builder b, BlockState blockState, boolean noSilkTouch, boolean error) {
            // Check for creative
        ...
        protected int findBlocksToPlace(Highway_Builder b) {
        ...
        protected int findBlocksToPlacePrioritizeTrash(Highway_Builder b) {
