    ...
    private void onTick(TickEvent.Post event) {
        for (Entity entity : mc.world.getEntities()) {
            ...
            if (mc.player.distanceTo(entity) < maxDist.get()) {
                mc.player.networkHandler.onDisconnect(new DisconnectS2CPacket(new LiteralText("[Anti Mob] " + Language.getInstance().get(entity.getType().toString()) + " got in your distance.")));
